...
const FRAMES_PER_LEVEL = [53, 49, 45, 41, 37, 33, 28, 22, 17, 11, 10, 9, 8, 7, 6, 6, 5, 5, 4, 4, 3];
...
let renderer = autoDetectRenderer({"width": CANVAS_WIDTH, "height": CANVAS_HEIGHT});
...
function render() {
	...
	for (let x = 0; x < GRID_WIDTH; x++) {
		for (let y = 0; y < GRID_HEIGHT; y++) {
			...
			if (grid[x][y].exists) {
				grid[x][y].sprite = Sprite.from("assets/blocks/" + grid[x][y].color + ".png");
