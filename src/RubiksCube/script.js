...
const ambientLight = new AmbientLight(0x333333, 50);
...
for (let x = 0; x < 3; x++) {
    for (let y = 0; y < 3; y++) {
        for (let z = 0; z < 3; z++) {
            ...
            cubes[9 * x + 3 * y + z] = new Mesh(
                new BoxGeometry(0.9, 0.9, 0.9, 15, 15, 15),
                cubeMaterialArray,
            );
...
function rotateReset() {
    ...
    for (let i = 0; i < cubes.length; i++) {
        ...
        cubes[i].rotation.x =
            (Math.round((cubes[i].rotation.x / Math.PI) * 2) * Math.PI) / 2;
        cubes[i].rotation.y =
            (Math.round((cubes[i].rotation.y / Math.PI) * 2) * Math.PI) / 2;
        cubes[i].rotation.z =
            (Math.round((cubes[i].rotation.z / Math.PI) * 2) * Math.PI) / 2;
    }
    activeRotations = 0;
}
function rotate(sideLetter, prime) {
    ...
    for (let i = 0; i < cubes.length; i++) {
        if (
            zPosition == null ||
            cubes[i].position.getComponent(zComponent) == zPosition
        ) {
            side.attach(cubes[i]);
        }
    }
